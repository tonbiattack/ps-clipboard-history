...
マウスで行をクリック、または上下矢印キーで行を選んで `Enter` を押すと、その内容をクリップボードへ戻し、履歴ウィンドウを自動で隠します。上下矢印キーだけではコピーしません。カウント列は表示しません。閉じるボタンはウィンドウを隠すだけで監視を続けます。通知領域の **Clipboard History** アイコンをダブルクリックまたは右クリック→「Open history」で再表示できます。`Ctrl + Alt + V` は最小化・非表示の状態からでも履歴ウィンドウを前面へ出し、検索欄にフォーカスします。右クリック→「Exit」で常駐を終了できます。`Win + V` は変更せず、Windows 標準のクリップボード履歴として併用します。
