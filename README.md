...
常駐アプリを起動すると、履歴ウィンドウは開いたままになります。`Ctrl + C` すると新しいコピー内容が自動で先頭に追加されます。マウスで行をクリックするとその内容をクリップボードへ戻します。上下矢印キーは選択を動かすだけでコピーしません。カウント列は表示しません。閉じるボタンはウィンドウを隠すだけで監視を続けます。通知領域の **Clipboard History** アイコンをダブルクリックまたは右クリック→「Open history」で再表示できます。右クリック→「Exit」で常駐を終了できます。`Ctrl + Alt + V` も補助として使えます。`Win + V` は変更せず、Windows 標準のクリップボード履歴として併用します。
