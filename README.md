...
常駐アプリを起動すると、履歴ウィンドウは開いたままになります。`Ctrl + C` すると新しいコピー内容が自動で先頭に追加されます。履歴一覧の上部にある検索欄へ文字を入力すると、クリップボード本文に対する大文字・小文字を区別しない部分一致で即時に絞り込めます。検索欄で下矢印キーを押すと一覧へ移動できます。

履歴一覧は読みやすさのため、12ptの文字・32pxの行高で表示します。

マウスで行をクリック、または上下矢印キーで行を選んで `Enter` を押すと、その内容をクリップボードへ戻します。コピー後も履歴ウィンドウは開いたままで、上下矢印キーだけではコピーしません。カウント列は表示しません。閉じるボタンはウィンドウを隠すだけで監視を続けます。通知領域の **Clipboard History** アイコンをダブルクリックまたは右クリック→「Open history」で再表示できます。`Ctrl + Alt + V` は最小化・非表示の状態からでも履歴ウィンドウを前面へ出し、検索欄にフォーカスします。右クリック→「Exit」で常駐を終了できます。`Win + V` は変更せず、Windows 標準のクリップボード履歴として併用します。
