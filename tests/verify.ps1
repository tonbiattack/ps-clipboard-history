...
Assert-That ($watcherSource -match '\[int\]::TryParse\(\$state\.NumberPrefix') 'long number input does not throw a conversion exception'
Assert-That ($watcherSource -match "Number selection cleared\.") 'Escape clears a pending number selection'
